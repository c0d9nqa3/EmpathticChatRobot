...
def gen_embeddings(vocab):
    ...
    if "vector/glove.6B.300d.txt" is not None:
        print('Loading embedding file: %s' % "vector/glove.6B.300d.txt")
        pre_trained = 0
        for line in open("vector/glove.6B.300d.txt", encoding='UTF-8').readlines():
            sp = line.split()
            if(len(sp) == 300 + 1):

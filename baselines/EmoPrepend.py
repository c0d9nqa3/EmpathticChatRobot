...
class EmoP(nn.Module):
    def __init__(self, vocab, decoder_number,  model_file_path=None, is_eval=False, load_optim=False):
        ...
        self.encoder = Encoder(300, 300, num_layers=1, num_heads=2, 
                                total_key_depth=40, total_value_depth=40,
                                filter_size=50,universal=False)


        ## multiple decoders
        self.decoder = Decoder(300, hidden_size = 300,  num_layers=1, num_heads=2, 
                                    total_key_depth=40,total_value_depth=40,
                                    filter_size=50)
        
        self.decoder_key = nn.Linear(300 ,decoder_number, bias=False)
        ...
        if False:
            self.optimizer = NoamOpt(300, 1, 8000, torch.optim.Adam(self.parameters(), lr=0, betas=(0.9, 0.98), eps=1e-9))
    ...
    def train_one_batch(self, batch, iter, train=True, loss_from_d=0.0):
        ...
        ## Embedding - context
        mask_src = enc_batch.data.eq(1).unsqueeze(1)  # (bsz, src_len)->(bsz, 1, src_len)
        ...
        sos_token = torch.LongTensor([3] * enc_batch.size(0)).unsqueeze(1)  # (bsz, 1)
        if False: sos_token = sos_token.cuda()
        dec_batch_shift = torch.cat((sos_token, dec_batch[:, :-1]), 1)  # (bsz, tgt_len)

        mask_trg = dec_batch_shift.data.eq(1).unsqueeze(1)
        ...
        logit = self.generator(pre_logit, attn_dist, enc_ext_batch if False else None, max_oov_length, attn_dist_db=None)
    ...
    def compute_act_loss(self,module):    
        ...
        loss = False_loss_weight * avg_p_t.item()
        return loss
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        ys = torch.ones(1, 1).fill_(3).long()  # test的时候 bsz = 1
        if False:
            ys = ys.cuda()
        ...
        for i in range(max_dec_step+1):
            ...
            decoded_words.append(['<EOS>' if ni.item() == 2 else self.vocab.index2word[ni.item()] for ni in next_word.view(-1)])

...
class Beam():
    ...
    def __init__(self, size, device=False):
        ...
        self.next_ys = [torch.full((size,), 1, dtype=torch.long, device=device)]
        self.next_ys[0][0] = 3
    ...
    def beam_search(self, batch, max_dec_step):
        ...
        def beam_decode_step(inst_dec_beams, len_dec_seq, src_seq, enc_output, inst_idx_to_position_map, n_bm,
            ...
            def predict_word(dec_seq, dec_pos, src_seq, enc_output, n_active_inst, n_bm, enc_batch_extend_vocab,
                ...
                ## masking
                mask_trg = dec_seq.data.eq(1).unsqueeze(1)
...
def evaluate(model, data, ty='valid', max_dec_step=30, adver_train=False):
    if adver_train is False:
        outputs = open("Predictions/{}.txt".format("EmpDG"), "w", encoding="utf-8")
    model.eval()
    model.cuda()
...
def gen_embeddings(vocab):
    ...
    """
    embeddings = np.random.randn(vocab.n_words, 300) * 0.01
    print('Embeddings: %d x %d' % (vocab.n_words, 300))
    if "vectors/glove.6B.300d.txt" is not None:
        print('Loading embedding file: %s' % "vectors/glove.6B.300d.txt")
        pre_trained = 0
        for line in open("vectors/glove.6B.300d.txt").readlines():
            sp = line.split()
            if(len(sp) == 300 + 1):
...
def share_embedding(vocab, pretrain=True):
    embedding = Embeddings(vocab.n_words, 300, padding_idx=1)
    if pretrain:
...
def get_input_from_batch(batch):
    ...
    c_t_1 = torch.zeros((batch_size, 2 * 300))

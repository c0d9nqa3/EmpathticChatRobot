...
def collate_fn(batch_data):
    ...
    d["context_lengths"] = torch.LongTensor(context_lengths).to(device)  # (bsz, )
    ...
    d["emotion_context_lengths"] = torch.LongTensor(emotion_context_lengths).to(device)  # (bsz, )
    d["mask_emotion_context"] = mask_emotion_context.to(device)

    ##output
    d["target_batch"] = target_batch.to(device)  # (bsz, max_target_len)
    d["target_ext_batch"] = target_ext_batch.to(device)
    d["target_lengths"] = torch.LongTensor(target_lengths).to(device)  # (bsz,)

    d["feedback_batch"] = feedback_batch.to(device)  # (bsz, max_target_len)
    d["feedback_lengths"] = torch.LongTensor(feedback_lengths).to(device)  # (bsz,)
    d["feedback_emotion_batch"] = feedback_emotion_batch.to(device)  # (bsz, max_target_len)

    ##program
    d["target_emotion"] = torch.LongTensor(item_info['emotion']).to(device)
    d["emotion_label"] = torch.LongTensor(item_info['emotion_label']).to(device)  # (bsz,)
    d["emotion_widx"] = torch.LongTensor(item_info['emotion_widx']).to(device)
...
def write_config():
    if not False:
        if not os.path.exists("save/EmpDG/"):
            os.makedirs("save/EmpDG/")
        with open("save/EmpDG/"+'config.txt', 'w') as the_file:
